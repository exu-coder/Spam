import requests, os, psutil, sys, jwt, pickle, json, binascii, time, urllib3, base64, datetime, re, socket, threading, ssl, pytz, aiohttp, random, logging
...
Chat_Leave = False
joining_team = False
# Auto update removed - using fixed values
...
# =============================================================================
# 🔥 OLD FUNCTIONS KEPT FOR BACKWARD COMPATIBILITY (app.py)
# =============================================================================

async def EncRypTMajoRLoGin(open_id, access_token):
    """Old function name kept for compatibility - now uses safe method"""
    return build_safe_major_login_payload(open_id, access_token)

async def MajorLogin(payload):
    """Old function name kept for compatibility - uses requests"""
    if not payload:
        return None
    headers = {
        'X-Unity-Version': '2018.4.11f1',
    ...
    try:
        response = requests.post(login_url + "/MajorLogin", headers=headers, data=payload, verify=False, timeout=15)
...
async def GetLoginData(base_url, payload, token):
    """GetLoginData - kept for app.py compatibility"""
...
def build_safe_major_login_payload(open_id, access_token):
    ...
    # Static encryption key/iv (same as acti.py)
...
async def safe_major_login_async(access_token, open_id):
    """Async wrapper for safe major login"""
    response = major_login_safe(access_token, open_id)
    if response:
        return response
    return None

async def safe_get_login_data_async(jwt_token, access_token, base_url):
    """Async wrapper for safe get login data"""
    return get_login_data_safe(jwt_token, access_token, base_url)
...
class CLIENT:
    ...
    async def MaiiiinE(self):
        ...
        # Use SAFE major login instead of old one
        major_response = await safe_major_login_async(access_token, open_id)
        if not major_response:
            print("TarGeT AccounT => BannEd / NoT ReGisTeReD ! ")
            return None
        
        MajoRLoGinauTh = await DecRypTMajoRLoGin(major_response)
        ...
        # Use SAFE get login data
        LoGinDaTa = await safe_get_login_data_async(ToKen, access_token, UrL)
        if not LoGinDaTa:
...
# =============================================================================
# 🔥 ASYNC FUNCTIONS FOR APP.PY COMPATIBILITY
# =============================================================================
