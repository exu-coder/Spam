...
def request_join_with_badge(target_uid, badge_value, key, iv, region="BD"):
    """Join request with badge using custom working avatars (from app.py)"""
    # Working avatar IDs from app.py's xBunnEr list
    avatar_ids = [
    ...
    fields = {
        ...
        2: {
            ...
            23: bytes([16, 1, 24, 1]), 24: selected_avatar,  # Working avatar
...
class FF_Client:
    ...
    def _full_auth(self):
        ...
        # We need to get the actual login data from the activation
        # Since activate_account doesn't return the data, we need to re-fetch
        # This is a limitation - but the activation process should set up the session
        
        # Actually, we need to get the credentials after activation
        # Let's try to get the token again and do the full login
        try:
    ...
    def send_all_spams(self, target_id):
        ...
        try:
            ...
            for i in range(5):
                ...
                time.sleep(0.03)
            # Badge join with working avatars (updated)
